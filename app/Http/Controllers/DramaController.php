...
class DramaController extends Controller
{
        ...
        public function browse(Request $request)
    {
        ...
        if ($request->filled('search')) {
            $dramas->where('title', 'like', '%' . $request->search . '%');
        }
        ...
        match ($sort) {
            ...
            'trending' =>
                $dramas->withAvg('userMediaLists', 'rating')
                        ->orderBy('user_media_lists_avg_rating', 'desc'),


            // Most Watched = paling banyak user
            'popular' =>
                $dramas->withCount('userMediaLists')
                        ->orderBy('user_media_lists_count', 'desc'),

            // Newest
            'newest' =>
                $dramas->orderBy('release_year', 'desc')->orderBy('title', 'asc'),


            default =>
                $dramas->orderBy('rating', 'desc'),
        };
        ...
        return view('drama.browse', compact('dramas', 'genres', 'sort'));
    }
}